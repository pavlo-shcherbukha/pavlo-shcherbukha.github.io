...
Як виявилося SPICE моделі на сайтах розробників компонентів дуже часто лежать поряд з DataShits на компонент. Тому для транзистора я її і використав. Лише додам, що 
...
### <a name="p4">4. Проблеми запуску та налагодження генераторів</a>
...
Основною проблемою  при запуску було те, що я не оцінив важливісь конденсатора С4 і поставив "що було під рукою", а саме неполярний 100nF. В результаті генератор видава 
прекрасний синус на частоті 25-26 kHz, але зміна коненсаторів фазо-зміщуйочого ланціюжка призводила до зриву генерації, а зміна резисторів в ланцюжку могла знизити 
...
Як видно, при живленні 9.2V (крона трошки відновилася) частота генерації 13.8 kHz. Тепер відключив позитивний зворотній зв'язок і  що на колекторі 
...
Але помітив, що із зменшенням напруги на колекторі транзистора збільшуються шуми на синусоїді, але генерація працює стабільно
...
Спершу я спробував змоделювати роботу підсилювача на транзисторі 2N2222 від тестового джерел змінного струму частотою  10kHz і амплітудою 0.01V. 
Ціль була вибрати  лінійний режим роботи транзистора. На pic-20 та pic-21  показано ідеальний режим роботи транзистора, коли амплітуда вихідного  
сигналу десь 1.6V  і ще є запас для росту при цьому значення резистора R3  ідеально мати  близько 25K.   
