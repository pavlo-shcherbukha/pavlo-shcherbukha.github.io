---
layout: post
title: "Fabric. Прагматичний AI та Цифрові Паспорти: Чому 5 рядків математики іноді цінніші за гігабайтні нейромережі"
...
# Fabric. Прагматичний AI та Цифрові Паспорти: Чому 5 рядків математики іноді цінніші за гігабайтні нейромережі
...
Давайте розберемо це на реальному прикладі: побудові системи моніторингу для промислового фармацевтичного холодильника та створенні Цифрового Паспорта Серії в Microsoft Fabric.
...
- З розроблених моделей сенсорів (симуляторів сенсорів), на яких налагоджувався весь процес. Архітектура симуляторів показана досить детально. І вній, на мій погляд, є кілька ключових елеменів,  гідних того, щоб на них звернути увагу.  На перше місце я б поставив те, що модель дозволяє запустити досить велику кількість сенсорів одночасно (в паралель) використовуючи особливості паралельної архітектури Spark. На друге місце, я б поставив те, що моделі можуть працювати в двої режимах: в режимі релаьного часу, що дає можливість вілагодити саме канал роботи реального часу та в режимі генерації даних за попередні періоди, що дає можливість швидко накопичити досить великий об'єм даних від сенсорів і перевірити продуктивність системи збереження при накопиченні значного об'єму даних. На третє місце я б поставив архітектурний підхід, коли спочтаку створюються робочі поведінокві профілі сеносорів, а потім ці профілі підключаються до сенсорів, що дає мложливість згенерувати та відлагодити аномальну поведінку сенсорів. Спершу, я використовував генерауцію даних сенсорів "на льоту" та ще і з підмішуванням випадкового шуму. Але я швидко відмовися від такої архітектури тільки тому, що мені потрібно генерувати аномалії передбачувані, щоб чітко відлагодити реакцію на них.

- З додатку на Raspberry PI, до якого з одної сторони підключено сенсор DHT22, а з іншої сторони встановлені python пакети для прямої передачі даних в черги Fabric Event Stream. Так як я використав Raspberry PI5 то основна складність була в установці "залізних" біблаотек та пакетів, що працюють з GPIO. Ті пакети, що багато раз згадувалися для  Raspberry PI-3B  - не підходять, навіть для останніх моделей Raspberry PI-3B. А підключеня до Fabric Event Stream виявилося типовим, як для Azure Event Hub. Із цікавого тут те, що процес передачі даних запускається зразу при включенні Raspberry PI "як сервіс" без будь якого впливу оператора, завдяки його запуску через System-d. На додаток в системний лог пишеться робота додатку. Ну і всі налаштування та секрети записані в ENV-змінних, що розглядається, як good practice. Зовнішній вигляд цього аапаратного  компонента показано на [pic-02](#pic-02).  
...
Тут споживаються RAW дані від датчиків. Як видно з [pic-01](#pic-01) система побудована за класичною архітектурою Fabric і складається з Fabric Event Stream (черги на кшталт Kafka) та time based бази даних Fabric Event hub (Kusto Database). В якості чогось близького можна згадати: InfluxDB, TimescaleDB. Але все ж таки Kusto Database на багато  потужніша і  адаптована під аналітику часових рядів (Time Series) у реальному часі. І вона вже має готові інструменти для виявлення аномалій (anomaly detection), прогнозування трендів (forecasting) та порівняння часових періодів, що ідеально підходить для обродки потокових даних від сенсорів. І уже прямо з Kusto Database ми будуємо дашбоарди для відображення RealTime аналітики і поряд же Activator для активації дій, при виявлені аномальної поведінки даних від сенсорів. В цій області ми не виконуємо складних обрахунків а більше готуємо та очищуємо дані для виконання  фінальних розрахунків, які переводять сирі дані датчиків зрозумілі бізнес сутності.

Тут треба згалади про Медальйону архітектуру Big Data. Так от Event hub (Kusto Database) в цьому контексті виступає як Bronze level та Silver level.

Потрібно зробити наголос і на тому, що вартість зберігання даних в Event hub (Kusto Database) дещо вища, ніж в Fabric lake house. Та і сам принцип збереження дещо інший ніж в Fabric lake house. Тому, не зважаючи на можливість зберігати в Event hub (Kusto Database) великі об'єми даних, порібно довготривале збереження даних виконувати в lake house. А на додаток, потрібно уникати зберігати довго Raw дані датчиків та "сирі" не очищені та не трансформовані дані. 
Все це треба обрахувати в бізнес сутності і перенести в Gold рівень на lake house.  Для цього служить  наступна область: "Aggregation area".

3. Область "Aggregation area" (область агрегації) даних.

Тут зберігаються  очищені та агреговані дані від датчиків, що перераховані вже в бізнес сутності, зрозумілі бізнесу. Якщо з вернутися до Медальйоної архітектіри, то це Gold level.


4. Область "Visualisation area" (область візуалізації) дозволяэ створити дашборди в PowerBi для візуалізації даних.

## <a name="p-2">Експеримент: «Пиво, фізика та датчик DHT22»</a>

Щоб зрозуміти характер об'єкта контролю, ми провели тридобовий R&D експеримент на базі звичайного домашнього холодильника, підключеного через Raspberry Pi 5 з датчиком температури DHT22 до KQL бази даних (Microsoft Fabric Real-Time Intelligence).  
...
На графіках уже відображається  gold рівень, коли покази сенсорів перераховані в показник MKT Mean Kinetic Temperature (середня кінетична температура), що широко викорситовується в фармацевтичній та харчовій промисловлсті.
...
• На початку (14 червня) під час тривалого спайку теплового навантаження («інцидент з пивом») МКТ різко злітає вгору — майже до 24 °C.
	
• Зверніть увагу, як повільно та плавно падає графік МКТ (порівняно з сирими даними): навіть коли фізична температура в холодильнику вже опустилася до норми (+4...+5 °C), МКТ залишається високою ще тривалий час (весь день 15 червня). Це ідеально моделює реальну кумулятивну шкоду для медикаментів від перегріву — система «пам'ятає» тепловий удар.

•  Також сліж звернути увагу на те, що на гррафіку MKT відсутні шуми сенсора, що чітко видно на [pic-03](#pic-03). Тож дані MKT вже більш придатні для якогось математичного аналізу.
...
- Визначення періоду компресора: Коли всередині з'являється теплоємний продукт (наше пиво), компресор працює і відпочиває довше — період коливань збільшується. Для його виявлення ідеально підходить Швидке перетворення Фур'є (FFT) або автокореляційна функція на ковзному вікні. Зміна головного піку на спектрограмі чітко сигналізує про завантаження камери.  
...
**По факту, це як покази магнітоелектричного вотльтметра змінного стуму, що показує середньо квадратичне значення амплітуди змінного струму**
...
А якщо серйозно, то Timeseries ряди даних від сенсорів (датчиків) еквівалентні радіотехнічним сигналам в оцифрованому вигляді з частото дискретизації що дорівньоє частоті надходження даних. А від так, для них справедливі всі закони і математичні апарати цифрової оброки сигналів та швидкого перетворення Фур'є. Що і було застосовано на даних реального холодильника , що записані в Microsoft Fabric. 
 
Таким чином, якщо ми хочемо вести якийсь prediction maintains роботи обладнання чи відловлювати аномалії роботи обладнання,  нам прийдеться працювати з аналізом цих "голкових" сплесків. Тому пішли шляхом цифрової обробки сигналів, пропустивши набір даних від датчика через цифровий фільтр високої частоти.  
...
Зробити міст між "важкою інженерною математикою" та бізнесом. Якщо  підключить Power BI до дельта-таблиць у Lakehouse, то отримаємо потужний інструмент для бізнес аналіз.
 
Що малювати в Power BI?

Коли ці дані опиняться в Lakehouse, розробнику не доведеться писати складні міри на DAX для виявлення аномалій, адже  вже зробили усю важку розрахункову роботу на рівні DSP (ЦОС)!
...
Аналітичний графік: Суміщений графік, де по осі X йде час, а по Y — реальна температура (синусоїда) та лінія  RMS. Цю "магыю" взагалі можна продати за штучний інтелект.

Я так думаю, що поєднання  низькоуровневих інженерних знань цифрової обробки сигналів із паралелізмом Spark у Fabric та візуалізацією Power BI — це і є створення істинного промислового цифрового двійника (Digital Twin).
...
Якщо вернутися до інженерної краси,  то треба подякувати великому колективу математиків, без якого ЦОС та швидке перетворення фур'є не народилося б:
...
І цей математичний апарта розроблений десь в часовому інтервалі від 1730 до 1845 років. Далі ідуть або методики викладання, або якісь нішеві винаходи.
