...
В цій архітектурі функція працює як  оркестратор, що реагує на події:
...
- Функція буде "спати" і очікуючи відповіді від запущеного процесу або вичерпання інтервалу очікування відповіді.  "Сон" означає те, що функція не витрачає процесорний час до тих пір, доки не надійде зовнішня подія.
Програмний код durablr function знаходиться в репозиторії [Dacha video Durable fuctionas - Оркестратор обчислень.](https://github.com/pavlo-shcherbukha/sh-py-ml-orchestrator-p/tree/master). Сам програмний код знаходиться в файлі: [function_app.py](https://github.com/pavlo-shcherbukha/sh-py-ml-orchestrator-p/blob/master/function_app.py).

Функція взаємодіє з чергами Azure Storage Queue.  Такми чином вимагається підключення до Storge Account де розгорнуі ті черги. Підключення до Storage account  відбувається за допомогою connect string. Для розробки це нормально. А от для продукту чи для продуктивної експлуатації потрібно робити підключення через **Menedget identity**.
...
Потрібнозвернути увагу на обробку помилок оркестраторі. В наведеному фрагменті кода оркестратора в блоці **Exception** для обробки помилки передбачено виклик такого ж **Activity**,  як і для основної бізнеслогіки. код самого Activity  наведено ниижче. 
...
Тепер розглянемо як обробляти зовнішню подію, як налаштувати відповідні тригери. На наведеному нижче фрагменті коду показан тригер, що слухає чергу Storage Queue за назвою **"video-processed**. Як можна помітити, що **client.raise_event(....)** приймає досить специфічні параметри:
...
Ще треба звернути увагу, що тригер на Storage Queue параметризується (тобто він уже існує і працює якась facory). Цей тригер очікує, що повідомлення в StorageQueue  пбулікується як base64 encoded. Відповідно, цей формат повинен наслідуватися і обробником, що публікує повідомлення в чергу. В іншому разі отримаємо помилку. 
