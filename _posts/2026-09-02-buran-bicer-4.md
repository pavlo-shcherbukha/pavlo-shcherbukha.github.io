...
- Як не дивно, "Дракон" отримав міжнародне визнання.
...
Порівнювати БЦВК «Бисер-4» з IBM AP-101 чи, тим паче, з домашніми ПК за тактовою частотою — це все одно, що порівнювати цифровий медичний кардіостимулятор зі смартфоном на Android. Смартфон швидший і має більше пам'яті, але кардіостимулятор працює роками без перезавантаження і рятує життя завдяки надійному алгоритму.

Якщо перекласти це мовою сучасного заліза:

- «Бисер-4» — це приблизно як чотири мікроконтролери ESP32, що працюють у жорсткій синхронній зв'язці.

- IBM AP-101 — приблизно як кластер із чотирьох основних і одного резервного одноплатного комп'ютера Raspberry Pi 3B під управлінням складної ОС.

Ніхто не порівнює ESP32 та Raspberry Pi лише за частотою — це пристрої для різних класів задач. Так само потрібно оцінювати не самі мікросхеми, а якість виконання завдання та логіку системи, у яку вони інтегровані.

Space Shuttle створювався як літак із можливістю ручного керування під контролем пілотів. «Буран» же був 100-тонним човником без посадкових двигунів: у нього не було другого кола й варіанту, коли людина могла б підстрахувати автоматику. Варіант був один — точний автоматичний розрахунок.

«Бисер-4» був саме таким спеціалізованим приладом. Його сильна сторона — не кількість транзисторів, а логіка управління, яка дозволила кораблю самостійно сісти в складних погодних умовах на Байконурі.

Якщо перейти до сьогодення, варто ще раз наголосити: польотна програма відпрацювала бездоганно передусім тому, що в її розробку та перевірку вклали величезні ресурси. Комплексний тестовий стенд давав змогу симулювати роботу ПЗ у майже реальних умовах.

Модульні та інтеграційні тести потрібні, але самі по собі не гарантують цілісної роботи системи в реальному агресивному середовищі. Повноцінне тестування можливе тоді, коли ще до передачі завдання розробникам є чіткий опис функціональності, а наскрізні сценарії тестування (E2E) готуються паралельно з основним кодом.

Якщо тести падають, це не привід казати: «Та давайте виходити в продакшн, там і протестуємо». Це підстава для ґрунтовного Root-Cause Analysis, тобто пошуку першопричин, і для підвищення загальної культури розробки. Коли замовник або виконавець заощаджує на цьому чи ставиться формально, наслідки не забаряться.

Показовий приклад — історія першого радянського супутника: його маса виявилася значно більшою за розрахункову, а деталі не поміщалися в конструкцію через відсутність належного вхідного та вихідного контролю якості у виробничому ланцюжку.
