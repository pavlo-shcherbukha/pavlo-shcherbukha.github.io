...
- LakeHouse with Delta Lake, по суті це універсальна opensource платформа збереження даних та побудови Data Lakes.
...
Основна ціль цього блогу спробувати пояснити, чому розробникам, що звикли проектувати додатки в середовищі корпоративних баз даних: ORACLE, MSSQL .... слід  серйозно віднестися до SPARK та LakeHouse with Delta Lake при проектуванні  додатків.
...
Далі, в цьому блозі, я спробую довести, чому слід серйьозно віднестися до зв'язки Spark та LakeHouse with Delta Lake розробниками корпоративних пробуктів навіть, якщо ви не готові використати Microsoft Fabric чи віддати свої дані в хмару взагалі. Ну і трошки поясню архітектуру даних LakeHouse with Delta Lake.
...
Я років 10 писав розрахунок і подачу звітності з комерційних банків в НБУ, та ще і на FOXPRO 2.0. Потім спробували все це перевести на ORACLE (тоді був уже версії 10). Там зіткнулися з проблемами, коли materialized VIEW агреговані і не агреговані працюють повільніше за FoxPro, який зливає всі необхідні дані на машину користувача, послідовно сканує таблиці і готує агреговані дані, а потім це повертає їх в мережеві файли dbf. Більш того, найнадійнішим варіантом в ORACLE було PL/SQL кодінг з деякою кількістю OLAP інструментів oracle та паралелізмом в запитах. Але там окрема проблема. Там запити кілометрові.
...
- Процес трансформації з оперативних даних в DWH виконується раз на день.
- Працює цей процес від години до 4 годин. Якщо більше, то це вже пробема взагалі.
- Цей процес за звичай запускається одним користувачем (часто службовим) і в ньому відстуні конкуретні користувачі.
...
Звичайно, досить легко можна підняти Spark  кластер в хмарі: Azure, IBM, AWS  і кожен з цих провайдерів має S3-сумісний Object Storage  для збереження Delta-файлів.
...
Зважаючи на те, що розрахунки робляться Jupyter Notebooks логічно було б і для ETL (ELT)  процесів використовувати цей інструмент. Ідея ця зовсім не нова, хоча я з нею познайомився вперше в Microsoft Fabric,  але потім знайшов багато продуктів, які використовують цей же підхід.
...
Oracle розробник не перестає бути розробником. Він просто розширяє свій горизонт іншими сучасними інстументами, виносячи обчислення, які не дуже притаманні базам данних на інші інстументи. Це, я б сказа, можливість дуже різкого переходу, що дає можливість осучаснити свої навички і зробити їх досить універсальними.
...
Більш того, я чомусть впевенний, що підсівши на ці інструменти., розробники і свою рутину можуть перевести на Jupyter Notebook (Jupyter Lab).

Для початку можна використати Porable Juptyter Lab [Winpython](https://winpython.github.io/). Тут Spark  немає, але дуже зручна платформа вивчити pyhton  кодинг і Jupyter Notebook.
